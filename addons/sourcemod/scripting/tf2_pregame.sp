...
#define SOUND_40 "vo/announcer_dec_kill09.mp3"
//#define SOUND_30 "vo/announcer_dec_missionbegins30s01.mp3"
#define SOUND_30 "vo/killstreak/announcer_ks_04.mp3"
#define SOUND_20 "vo/announcer_dec_kill12.mp3"
//#define SOUND_10 "vo/announcer_dec_missionbegins10s01.mp3"
#define SOUND_10 "vo/killstreak/announcer_ks_11.mp3"
#define SOUND_5 "vo/announcer_begins_5sec.mp3"
#define SOUND_4 "vo/announcer_begins_4sec.mp3"
#define SOUND_3 "vo/announcer_begins_3sec.mp3"
#define SOUND_2 "vo/announcer_begins_2sec.mp3"
#define SOUND_1 "vo/announcer_begins_1sec.mp3"

#define SOUND_START_1 "vo/announcer_am_gamestarting05.mp3"
#define SOUND_START_2 "vo/announcer_am_roundstart02.mp3"
#define SOUND_START_3 "vo/announcer_am_roundstart04.mp3"
#define SOUND_START_4 "vo/announcer_dec_missionbegins30s04.mp3"
#define SOUND_START_5 "vo/announcer_dec_missionbegins30s05.mp3"

#define SOUND_END_1 "vo/announcer_tournament_started3.mp3"
#define SOUND_END_2 "vo/announcer_am_killstreak06.mp3"
#define SOUND_END_3 "vo/announcer_am_lastmanalive01.mp3"
#define SOUND_END_4 "vo/announcer_am_lastmanalive04.mp3"
#define SOUND_END_5 "vo/announcer_dec_success01.mp3"
#define SOUND_END_6 "vo/announcer_dec_success02.mp3"
...
new Handle:g_hVarSpeak = INVALID_HANDLE;
...
new Handle:g_hVarDoors = INVALID_HANDLE,
	...
	Handle:g_hVarRespawn = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	g_hVarTime = CreateConVar("tf2_pregame_timelimit", "50", "time in seconds that pregame lasts", 0, true, 10.0, false);
	g_hVarStats = CreateConVar("tf2_pregame_stats", "1", "Track the number of kills people get during", 0, true, 0.0, true, 1.0);
	g_hVarDoors = CreateConVar("tf2_pregame_doors", "3", "1 = open all map doors.\n2 = close and lock all map doors.\n3 = random.\n0=Do not mess with doors",
								0, true, 0.0, true, 2.0);
	g_hVarCrits = CreateConVar("tf2_pregame_crits", "3", "0 = no crits.\n1 = Mini Crits\n2 = Full crits.\n3 = random.", 0, true, 0.0, true, 3.0);
	g_hVarMelee	= CreateConVar("tf2_pregame_melee", "2", "0 = allow all weapons.\n1 = Melee only.\n2 = random", 0, true, 0.0, true, 2.0);
	g_hVarRespawn = CreateConVar("tf2_pregame_respawn", "0.6", "Delay before respawning", 0, true, 0.0, true, 5.0);
	g_hVarSpeak = CreateConVar("tf2_pregame_speak", "1", "Have the Announcer count down the remaining time.", 0, true, 0.0, true, 1.0);

	CreateConVar("sm_pregame_slaughter_version", PL_VERSION, "Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	
	RegConsoleCmd("tf2_pregame_test", Test_Mayhem);
...
public Menu_CookieSettingsEnable(Handle:menu, MenuAction:action, param1, param2)
{
	...
	if (action == MenuAction_Select) 
	{
		...
		if (StrEqual(sSelection, "enable", false))
		{
			...
			CPrintToChat(client, "{azure}[{whitesmoke}Mayhem{azure}]{white} Winner menu ENABLED");
		}
		else
		{
			SetClientCookie(client, g_hCookie_ClientMenu, "-1");
			g_iMenuSettings[client] = -1;
			CPrintToChat(client, "{azure}[{whitesmoke}Mayhem{azure}]{white} Winner menu DISABLED");
		}
	}
	else if (action == MenuAction_Cancel) 
	{
...
public Event_PlayerDeath(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (g_bPreGame)
	{
		...
		CreateTimer(GetConVarFloat(g_hVarRespawn), Timer_Spawn, iUserId);
	}
}
...
public Action:Event_Inventory(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (client && IsPlayerAlive(client))
	{
		...
		CreateTimer(0.0, Timer_SetCond, GetClientUserId(client));
	}
	return Plugin_Handled;
}
...
stock StartPreGame()
{
	CPrintToChatAll("{azure}[{whitesmoke}Mayhem{azure}]{white}: {firebrick}Round Started");
	...
	RespawnAll();
	CreateTimer(0.1, Timer_PlayStartSound);	
	
	CreateTimer(4.0, Timer_DisableLockers);
	
	new iCountdown = GetConVarInt(g_hVarTime);
	SetConVarInt(FindConVar("mp_waitingforplayers_time"), iCountdown);
	g_currentCounter = CreateTimer(1.0, CountdownTimer, iCountdown-3, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
...
stock StopPreGame()
{
	if (g_bPreGame)
	{
		CPrintToChatAll("{azure}[{whitesmoke}Mayhem{azure}]{white}: {firebrick}Round Over");
...
public Action:Timer_Winners(Handle:timer)
{
	...
	if (iRedCount && iBluCount)
	{
		...
		SetPanelTitle(hMenu, "Mayhem Round Results");
		DrawPanelItem(hMenu, "", ITEMDRAW_SPACER|ITEMDRAW_RAWLINE);
		
		DrawPanelItem(hMenu, "RED Team Winners", ITEMDRAW_RAWLINE);
		for (new i; i < 3; i++)
		{
			if (IsClientInGame(iRedScores[i][0]))
			{
				GetClientName(iRedScores[i][0], sNameBuffer, sizeof(sNameBuffer));
				Format(sBuffer, sizeof(sBuffer), "%i.  %s  %i Frags", i+1, sNameBuffer, iRedScores[i][1]);
				DrawPanelItem(hMenu, sBuffer, ITEMDRAW_RAWLINE);
			}
		}
		
		DrawPanelItem(hMenu, "", ITEMDRAW_SPACER|ITEMDRAW_RAWLINE);
		DrawPanelItem(hMenu, "BLU Team Winners", ITEMDRAW_RAWLINE);
		for (new i; i < 3; i++)
		{
			if (IsClientInGame(iBluScores[i][0]))
			{
				GetClientName(iBluScores[i][0], sNameBuffer, sizeof(sNameBuffer));
				Format(sBuffer, sizeof(sBuffer), "%i.  %s  %i Frags", i+1, sNameBuffer, iBluScores[i][1]);
				DrawPanelItem(hMenu, sBuffer, ITEMDRAW_RAWLINE);
			}
		}
		...
		if (IsClientInGame(iRedScores[0][0]))
		{
			new i = iRedScores[0][0];
			
			GetClientName(i, sNameBuffer, sizeof(sNameBuffer));
			Format(sBuffer, sizeof(sBuffer), "{azure}[{whitesmoke}Mayhem{azure}]{white}: {red}RED Team Winner - {limegreen}%s", sNameBuffer);
		...
		if (IsClientInGame(iBluScores[0][0]))
		{
			new i = iBluScores[0][0];
			
			GetClientName(i, sNameBuffer, sizeof(sNameBuffer));
			Format(sBuffer, sizeof(sBuffer), "{azure}[{whitesmoke}Mayhem{azure}]{white}: {blue}BLU Team Winner - {limegreen}%s", sNameBuffer);
			CPrintToChatAll(sBuffer);
...
public Action Test_Mayhem(int client, intargs)
{		
	decl String:sNameBuffer[MAX_NAME_LENGTH+1];
	decl String:sBuffer[255];
	...
	for (new i; i < 3; i++)
	{
		Format(sBuffer, sizeof(sBuffer), "%i. x  # Frags", i+1);
		DrawPanelItem(hMenu, sBuffer, ITEMDRAW_RAWLINE);
	}
...
public OnMapStart()
{
	...
	PrecacheSound(SOUND_40);
	PrecacheSound(SOUND_30);
	PrecacheSound(SOUND_20);
	...
	PrecacheSound(SOUND_START_1);
	PrecacheSound(SOUND_START_2);
	PrecacheSound(SOUND_START_3);
	PrecacheSound(SOUND_START_4);
	PrecacheSound(SOUND_START_5);
	PrecacheSound(SOUND_END_1);
	PrecacheSound(SOUND_END_2);
	PrecacheSound(SOUND_END_3);
	PrecacheSound(SOUND_END_4);
	PrecacheSound(SOUND_END_5);
	PrecacheSound(SOUND_END_6);
...
public Action:CountdownTimer(Handle:timer, any:timerMaxTime)
{
	...
	//CPrintToChatAll("Countdown: %d", timeRemaining);
	
	if (GetConVarBool(g_hVarSpeak))
	{
	...
	if (timePassed++ >= timerMaxTime)
	{
		...
		{		
			...
			switch (i)
			{
				case 0:
				{
					EmitSoundToAll(SOUND_END_1);
				}
				case 1:
				{
					EmitSoundToAll(SOUND_END_2);
				}
				case 2:
				{
					EmitSoundToAll(SOUND_END_3);
				}
				case 3:
				{
					EmitSoundToAll(SOUND_END_4);
				}
				case 4:
				{
					EmitSoundToAll(SOUND_END_5);
				}
				case 5:
				{
					EmitSoundToAll(SOUND_END_6);
				}
			}
		}
...
public Action:Timer_PlayStartSound(Handle:timer, any:client)
{
	if (GetConVarBool(g_hVarSpeak))
	{
		new i = GetRandomInt(0, 4);
		switch (i)
		{
			case 0:
			{
				EmitSoundToAll(SOUND_START_1);
			}
			case 1:
			{
				EmitSoundToAll(SOUND_START_2);
			}
			case 2:
			{
				EmitSoundToAll(SOUND_START_3);
			}
			case 3:
			{
				EmitSoundToAll(SOUND_START_4);
			}
			case 4:
			{
				EmitSoundToAll(SOUND_START_5);
			}
		}
	}
...
public Action:Timer_DisableLockers(Handle:timer, any:client)
{	
	ModifyLockers("disable");	
	return Plugin_Handled;
}
