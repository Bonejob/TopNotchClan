...
{
	RegAdminCmd("sm_botmanager", Command_ToggleBotManager, ADMFLAG_GENERIC, "[1/0] Enable/Disable Bot Manager");
	...
	cvarBotsEnabled = CreateConVar("sm_bots_enabled", "1", "[1/0] Enable/Disable Bot Manager", 0, true, 0.0, true, 1.0);
	...
	HookConVarChange(buffer, OnConVarChange);
	HookConVarChange(cvarBotsEnabled, OnConVarChange);	
	
	botman_enabled = GetConVarInt(cvarBotsEnabled);
}
...
public OnConVarChange(Handle:convar, const String:oldValue[], const String:newValue[]) {
	...
	else if(StrEqual(name, "tf_bot_quota_mode")) {
...
public Action:Command_ToggleBotManager(client, args)
{	
	if(!botman_enabled)
		ServerCommand("sm_bots_enabled", 1);
	else
		ServerCommand("sm_bots_enabled", 0);
...
DisplayAdminBotManagerMenu(client)
{
	...
	if(botman_enabled)
		menuDescription = "DISABLE Bot Manager";
	else
		menuDescription = "ENABLE Bot Manager";

	AddMenuItem(menu,"Option 1",	menuDescription);
	AddMenuItem(menu,"Option 2",	"Increase Bot Quota");
	AddMenuItem(menu,"Option 3",	"Decrease Bot Quota");
		...
		switch (action)
		{
			case MenuAction_Select:
			{
				switch (param2)
				{
					...
					case 1:
					{
						ServerCommand("sm_bot_quota %i", GetConVarInt(cvarBotQuota)+2);					
					}

					case 2:
					{
						ServerCommand("sm_bot_quota %i", MAX(GetConVarInt(cvarBotQuota)-2, 0));	
					}
				}
			}
		}
