...
new Handle:Cvar_AlltalkManager = INVALID_HANDLE
...
public OnPluginStart()
{
	...
	Cvar_AlltalkManager = FindConVar("satm_enabled")
	...
	RegAdminCmd("sm_alltalkmanager", admin_setalltalkmanager, ADMIN_CHAT, "sm_alltalkmanager - Sets Simple All Talk Manager On or Off")
...
public Action:admin_setalltalk(client, args)
{
	if (GetConVarInt(Cvar_Alltalk) == 0)
	{
		...
		PrintToChatAll("\x01\x04[SM] All Talk\x01: Enabled by admin")
		
		SetConVarInt(Cvar_AlltalkManager, 0, false, true)
	}
	else
	{
		ServerCommand("sv_alltalk 0")
		PrintToChatAll("\x01\x04[SM] All Talk\x01: Disabled by admin")
		
		SetConVarInt(Cvar_AlltalkManager, 1, false, true)
	}
	return Plugin_Handled
}

public Action:admin_setalltalkmanager(client, args)
{
	if (GetConVarInt(Cvar_AlltalkManager) == 0)
	{
		SetConVarInt(Cvar_AlltalkManager, 1, false, true)
	}
	else
...
public Action:admin_setwarmode(client, args)
{
	if (GetConVarInt(Cvar_WarMode) == 0)
	{
		...
		PrintToChat(client,"\x01\x04[AdminSentinel]\x01: War mode disabled")
	}
	else
	{
		SetConVarInt(Cvar_WarMode, 0, false, true)
		PrintToChat(client,"\x01\x04[AdminSentinel]\x01: War mode enabled")
	}
	return Plugin_Handled
}
		...
		{		
			if ((GetUserFlagBits(client) & ADMIN_WAR) || (GetUserFlagBits(client) & ADMFLAG_ROOT))
			{
				if(g_EnemyVoice[client] == 1)
				{
					g_EnemyVoice[client] = 0
					SetClientListeningFlags(client, SPEAK_NORMAL)
					PrintToChat(client,"\x01\x04[AdminSentinel]\x01: Listening to enemy voice comm disabled")
					return Plugin_Handled
				}
				else if(g_EnemyVoice[client] == 0)
				{
					g_EnemyVoice[client] = 1
					SetClientListeningFlags(client, SPEAK_LISTENALL)
					PrintToChat(client,"\x01\x04[AdminSentinel]\x01: Listening to enemy voice comm enabled")
					return Plugin_Handled
				}
			}
			else
	...
	if ((client > 0) && IsClientInGame(client))
	{
		if (GetConVarInt(Cvar_WarMode) == 1)
		{
			if ((GetUserFlagBits(client) & ADMIN_WAR) || (GetUserFlagBits(client) & ADMFLAG_ROOT))
			{
				...
				{
					g_EnemyChat[client] = 0
					PrintToChat(client,"\x01\x04[AdminSentinel]\x01: Listening to enemy chat disabled")
					return Plugin_Handled
				}
				else if(g_EnemyChat[client] == 0)
				{
					g_EnemyChat[client] = 1
					PrintToChat(client,"\x01\x04[AdminSentinel]\x01: Listening to enemy chat enabled")
					return Plugin_Handled
				}
			}	
			else
		...
		if (GetConVarInt(Cvar_WarMode) == 1)
		{
			if ((GetUserFlagBits(client) & ADMIN_WAR) || (GetUserFlagBits(client) & ADMFLAG_ROOT))
			{
				if(g_AdminTalk[client] == 1 && g_EnemyVoice[client] == 0)
				{
					...
					SetClientListeningFlags(client, SPEAK_NORMAL)
					PrintToChat(client,"\x01\x04[AdminSentinel]\x01: Talk to Server is off")
					return Plugin_Handled
				}
				...
				else if(g_AdminTalk[client] == 0)
				{
					...
					PrintToChat(client,"\x01\x04[AdminSentinel]\x01: Talk to Server is on")
					return Plugin_Handled
				}
			}
			else
...
public AdminMenu_AllTalkManager(Handle:topmenu, 
			TopMenuAction:action,
			TopMenuObject:object_id,
			param,
			String:buffer[],
			maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
		if (GetConVarInt(Cvar_AlltalkManager) == 0)
		{
			Format(buffer, maxlength, "Turn ON All Talk Manager")
		}
		else
		{
			Format(buffer, maxlength, "Turn OFF All Talk Manager")
		}
	}
	else if (action == TopMenuAction_SelectOption)
	{
		admin_setalltalkmanager(param, ADMIN_LEVEL|ADMFLAG_ROOT)
	}
}
		...
		for (new i = 1; i <= numofPlayers; i++)
		{
			if (IsClientInGame(i))
			{
				if(g_EnemyVoice[i] == 1)
				{
					...
					PrintToChat(i,"\x01\x04[AdminSentinel]\x01: Listening to enemy voice comm re-enabled")
				}
			}
		}
	}
		...
		for (new i = 1; i <= numofPlayers; i++)
		{
			if (IsClientInGame(i))
			{
				if ((GetUserFlagBits(i) & ADMIN_WAR) || (GetUserFlagBits(i) & ADMFLAG_ROOT))
				{
					...
					{
						SetClientListeningFlags(i, SPEAK_LISTENALL)
						PrintToChat(i,"\x01\x04[AdminSentinel]\x01: War Mode on. Listening to enemy voice comm enabled")
					}
				}
				else if (GetUserFlagBits(i) & ADMIN_LEVEL)
				{
					SetClientListeningFlags(i, SPEAK_NORMAL)
					PrintToChat(i,"\x01\x04[AdminSentinel]\x01: War Mode on.  Listening to enemy voice comm disabled")
				}
			}
		}
	}
		...
		for (new i = 1; i <= numofPlayers; i++)
		{
			if (IsClientInGame(i))
			{
				if (GetUserFlagBits(i) & ADMIN_LEVEL)
				{
					...
					{
						SetClientListeningFlags(i, SPEAK_LISTENALL)
						PrintToChat(i,"\x01\x04[AdminSentinel]\x01: War Mode off. Listening to enemy voice comm enabled")
					}
				}
			}
		}
...
{	
	...
	if (strcmp(text[startidx], "at", false) == 0 || strcmp(text[startidx], "/at", false) == 0 || strcmp(text[startidx], "alltalk", false) == 0)
	{
		if (Cvar_Alltalk != INVALID_HANDLE)
		{
			...
			PrintToChatAll("\x01\x04[SM]\x01: %s", message);
		}
	}
	/* Let say continue normally */
	return Plugin_Continue
