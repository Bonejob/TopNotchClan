	...
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			switch (iSelection)
			{
				...
				case 3:
				{
					ShowMOTDPanel(client, "PANEL_TITLE", "http://www.topnotchclan.com/forums/showthread.php?t=813", MOTDPANEL_TYPE_URL);
				}
			}
		}		
	}
...
public Action:Command_Admins(client, args)
{
	...
	if (id != INVALID_ADMIN_ID)
	{
		...
		DrawPanelItem(panel, "Bonus Round Settings");
...
public PanelHandler(Handle:menu, MenuAction:action, param1, param2)
{
	if(action == MenuAction_Select)
	{
		if(param2 == 1) //Client saw the menu and decided to configure the settings.
		{
			if(g_donator[param1])
			{
				ShowMOTDPanel(param1, "PANEL_TITLE", "http://topnotchclan.com/forums/showthread.php?t=829", MOTDPANEL_TYPE_URL);
			}
			else
			{
				ShowMOTDPanel(param1, "PANEL_TITLE", "http://www.topnotchclan.com/forums/showthread.php?t=813", MOTDPANEL_TYPE_URL);
			}
		}
...
stock Database_Init()
{
	...
	g_Database = SQL_Connect("vbulletin", true, error, sizeof(error));
	if(g_Database == INVALID_HANDLE)
	{
		PrintToServer("[Connect-Announce] Could not use feedback database config, trying default.");
	...
	if(g_Database != INVALID_HANDLE)
	{
		...
		PrintToServer("[Connect-Announce] Connected successfully.");
		return true;
	} 
	else 
	{
		PrintToServer("[Connect-Announce] Connection Failed: %s", error);
		return false;
	}
}