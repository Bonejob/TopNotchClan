 ...
 */
 ...
 #define PLUGIN_NAME	"Group Inviter"
 #define PLUGIN_URL		"www.topnotchclan.com"
 #define PLUGIN_DESCR	"Invite players to a group"
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_vipinvite", Command_VIPInvite, ADMFLAG_RESERVATION, "Invites you into our VIP steam group");
	// Create Commands
...
/* Plugin Logic */
public Action:Command_Invite(hClient, iArgs)
{
	if (bEnable)
	{
		...
		PrintToChat(hClient, "\x04Invite has been sent! Check your steam community page to accept it!");
	}
	return Plugin_Handled;
}

public Action:Command_VIPInvite(hClient, iArgs)
{
	if (bEnable)
	{
		new Handle:hSocket = SocketCreate(SOCKET_TCP, OnSocketError);
		SocketSetArg(hSocket, hClient);
		SocketConnect(hSocket, OnSocketConnectedVIP, OnSocketReceive, OnSocketDisconnected, "topnotchclan.com", 80);
		PrintToChat(hClient, "\x04Invite has been sent! Check your steam community page to accept it!");
	}
	return Plugin_Handled;
}
...
public OnSocketConnectedVIP(Handle:socket, any:hClient) {
	decl String:sRequestStr[256];
	decl String:sAuth[256];
	GetClientAuthString(hClient, sAuth, sizeof(sAuth));
	Format(sRequestStr, sizeof(sRequestStr), "GET /server/vipinviter.php?key=%i&auth=%s HTTP/1.0\r\nHost: topnotchclan.com\r\nConnection: close\r\n\r\n", iKey, sAuth);
	SocketSend(socket, sRequestStr);
}
