...
*************************************************************************
*
* 1.3.9r - Fixed issue of not getting client cookie color when the DisableColor None cvar was enabled.
...
public OnPluginStart()
{
	...
	HookEvent("player_sapped_object", HookPlayerSappedObject, EventHookMode_Post);
...
public HookPlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(iClient < 1 || !IsPlayerAlive(iClient) || !g_bIsPlayerAdmin[iClient])
	return;

	if(!g_bDonorAdvertShown[iClient])
	{
		CreateTimer(0.5, Timer_DonorCookieAdvert, iClient, TIMER_FLAG_NO_MAPCHANGE);
	}

	if(!g_bRoundEnd)
	return;

	if(g_aClientCookies[iClient][bEnabled])
	{
		EnableImmunity(iClient);
	}
}

public HookPlayerSappedObject(Handle:event, const String:name[], bool:dontBroadcast)
{
...
stock CleanUp(iClient)
{
	...
	g_bPlayerHasPowerplay[iClient] = false;
...
stock DisableImmunity(iClient)
{
	...
	if (IsClientInGame(iClient))
	{
		...
		if(g_bPlayerHasPowerplay[iClient])
		{
...
stock AttachPlayerParticles(iClient)
{
	//PrintToChatAll("ClientCookieParticle is: %i", g_aClientCookies[iClient][iParticle]);
	switch(g_aClientCookies[iClient][iParticle])
	{
	case PARTICLE_ESSENCE:
		{
			AttachDonorParticle(iClient, "player_dripsred", 0.0, ATTACH_HEAD, 0);
			AttachDonorParticle(iClient, "player_drips_blue", 0.0, ATTACH_HEAD, 1);
			AttachDonorParticle(iClient, "coin_large_blue", 10.0, ATTACH_NORMAL, 2);
			AttachDonorParticle(iClient, "coin_large_blue", 10.0, ATTACH_NORMAL, 3);
			AttachDonorParticle(iClient, "medicgun_beam_attrib_muzzle", 0.0, ATTACH_HEAD, 4);
			AttachDonorParticle(iClient, "ghost_glow", 15.0, ATTACH_NORMAL, 5);
		}
	case PARTICLE_ELECTROSHIELD:
		{
			AttachDonorParticle(iClient, "critgun_weaponmodel_blu", 5.0, ATTACH_NORMAL, 0);
			AttachDonorParticle(iClient, "crutgun_firstperson", 40.0, ATTACH_NORMAL, 1);
			AttachDonorParticle(iClient, "crutgun_firstperson", 40.0, ATTACH_NORMAL, 2);
			AttachDonorParticle(iClient, "crutgun_firstperson", 40.0, ATTACH_NORMAL, 3);
			AttachDonorParticle(iClient, "crutgun_firstperson", 40.0, ATTACH_NORMAL, 4);
			AttachDonorParticle(iClient, "crutgun_firstperson", 40.0, ATTACH_NORMAL, 5);
		}
	case PARTICLE_FLARETRAIL:
		{
			AttachDonorParticle(iClient, "flaregun_trail_crit_red", 15.0, ATTACH_NORMAL, 0);
			AttachDonorParticle(iClient, "flaregun_trail_crit_red", 15.0, ATTACH_NORMAL, 1);
			AttachDonorParticle(iClient, "flaregun_trail_crit_blue", 15.0, ATTACH_NORMAL, 2);
			AttachDonorParticle(iClient, "flaregun_trail_crit_blue", 15.0, ATTACH_NORMAL, 3);
		}
	case PARTICLE_FIRESTARTER:
		{
			AttachDonorParticle(iClient, "buildingdamage_fire3", 0.0, ATTACH_HEAD, 0);
			AttachDonorParticle(iClient, "tpdamage_4", 10.0, ATTACH_NORMAL, 1);
			AttachDonorParticle(iClient, "burningplayer_red", 10.0, ATTACH_NORMAL, 2);
			AttachDonorParticle(iClient, "burningplayer_red", 10.0, ATTACH_NORMAL, 3);
			AttachDonorParticle(iClient, "critical_rocket_red", 10.0, ATTACH_NORMAL, 4);
		}
	case PARTICLE_BUBBLETRAIL:
		{
			AttachDonorParticle(iClient, "rockettrail_waterbubbles", 5.0, ATTACH_NORMAL, 0);
			AttachDonorParticle(iClient, "rockettrail_waterbubbles", 5.0, ATTACH_NORMAL, 1);
			AttachDonorParticle(iClient, "env_rain_splashring", 5.0, ATTACH_NORMAL, 2);
			AttachDonorParticle(iClient, "env_rain_splashripple", 5.0, ATTACH_NORMAL, 3);
			AttachDonorParticle(iClient, "env_rain_splashripple", 5.0, ATTACH_NORMAL, 4);
		}
	case PARTICLE_SPOOKED:
		{
			AttachDonorParticle(iClient, "ghost_glow", 30.0, ATTACH_NORMAL, 0);
			AttachDonorParticle(iClient, "yikes_fx", 0.0, ATTACH_HEAD, 1);
			AttachDonorParticle(iClient, "ghost_appearation", 0.0, ATTACH_NORMAL, 2);
		}
	case PARTICLE_POWERPLAY:
		{
			TF2_SetPlayerPowerPlay(iClient, true);
			g_bPlayerHasPowerplay[iClient] = true;
		}
	case PARTICLE_GLOW:
		{
			...
			if(team == 2)
			{
				AttachDonorParticle(iClient, "player_glowred", 5.0, ATTACH_NORMAL, 0);
				AttachDonorParticle(iClient, "player_glowred", 5.0, ATTACH_NORMAL, 1);
				AttachDonorParticle(iClient, "soldierbuff_red_volume", -5.0, ATTACH_NORMAL, 2);
				AttachDonorParticle(iClient, "soldierbuff_red_volume", -5.0, ATTACH_NORMAL, 3);
				AttachDonorParticle(iClient, "soldierbuff_red_volume", -5.0, ATTACH_NORMAL, 4);
			}
			else
			{
				AttachDonorParticle(iClient, "player_glowblue", 5.0, ATTACH_NORMAL, 0);
				AttachDonorParticle(iClient, "player_glowblue", 5.0, ATTACH_NORMAL, 1);
				AttachDonorParticle(iClient, "soldierbuff_blue_volume", -5.0, ATTACH_NORMAL, 2);
				AttachDonorParticle(iClient, "soldierbuff_blue_volume", -5.0, ATTACH_NORMAL, 3);
				AttachDonorParticle(iClient, "soldierbuff_blue_volume", -5.0, ATTACH_NORMAL, 4);
			}
		}
	case PARTICLE_NONE:
		{
...
public EnabledChanged(Handle:convar, const String:oldValue[], const String:newValue[])
{
	if(convar == Cvar_bri_enabled)
	{
		if (StringToInt(newValue) == 0) 
		{
			...
			UnhookEvent("player_sapped_object", HookPlayerSappedObject, EventHookMode_Post);
			for (new x = 1; x <= MaxClients; x++) 
			{
