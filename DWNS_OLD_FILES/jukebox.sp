...
public OnPluginStart() {
	...
	cv_db_conn_name = CreateConVar("jb_db_conn_name", "jukebox", "Named SQL connection to be used by Jukebox.  Database must be defined within sourcemod/configs/database.cfg", FCVAR_SPONLY|FCVAR_PRINTABLEONLY|FCVAR_PLUGIN);
	...
	for(new i = 0; i < update_count; i++) {
		if(now_volume[clients_list[i]] < 0) {
			...
			if(temp_volume > 0) {
				...
				new Handle:hData; 
				CreateDataTimer(1.0, data_callback, hData);
				WritePackCell(hData, sizeof(play_url));
				WritePackString(hData, play_url);				
				WritePackCell(hData, clients_list[i]);
				StopMOTD(clients_list[i]);
			}
		}
	}
...
public Action:data_callback(Handle:timer, any:hData)
{
	ResetPack(hData);
	new ArrSize = ReadPackCell(hData),
		String:sUrl[250];
	ReadPackString(hData, sUrl, ArrSize);
	new client = ReadPackCell(hData);
	if (IsClientInGame(client))
	{	
		LoadMOTDPanelHidden(client, "Jukebox - Source Engine Streaming Music System", sUrl, MOTDPANEL_TYPE_URL);
	}
	return Plugin_Handled;
}
