	...
	for(new i = 1; i<= MaxClients; i++) {
		...
		if(GetPlayerEye(i, pos)) {
			for(new a=1; a<=MaxClients;a++) {
				if(IsClientInGame(a) && GetVectorDistance(pos, SprayTrace[a]) <= GetConVarFloat(g_cvars[MAXDIS])) {
						...
						if(CanHUD && GetConVarBool(g_cvars[USEHUD])) {
							...
							if(!HasChangedHud) {
								...
								if(GetClientTeam(a) == 2) {
									SetHudTextParams(0.04, 0.6, GetConVarFloat(g_cvars[HUDTIME]), 255, 63, 53, 255);
									}
								if(GetClientTeam(a) == 3) {
									SetHudTextParams(0.04, 0.6, GetConVarFloat(g_cvars[HUDTIME]), 153, 204, 255, 255);
									}
								if(GetClientTeam(a) == 1) {
									SetHudTextParams(0.04, 0.6, GetConVarFloat(g_cvars[HUDTIME]), 90, 90, 90, 255);
									}
							}
...
public OnAdminMenuReady(Handle:topmenu) {
	...
	AddToTopMenu(hTopMenu, "sm_spraytrace", TopMenuObject_Item, AdminMenu_TraceSpray, server_commands, "sm_spraytrace", ADMFLAG_BAN);
	AddToTopMenu(hTopMenu, "sm_removespray", TopMenuObject_Item, AdminMenu_SprayRemove, server_commands, "sm_removespray", ADMFLAG_BAN);
	AddToTopMenu(hTopMenu, "sm_adminspray", TopMenuObject_Item, AdminMenu_AdminSpray, server_commands, "sm_adminspray", ADMFLAG_BAN);
}
