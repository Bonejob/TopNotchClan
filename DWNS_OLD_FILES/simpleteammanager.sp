...
#define PLUGIN_VERSION "3/16/2013"
...
#define VOTE_YES "##YES##"
#define VOTE_NO "##NO##"
#define Shame "TopNotchClan/shame.mp3"
...
public OnPluginStart()
{
	...
	//RegConsoleCmd("sm_forcesuddendeath", Command_SuddenDeath, "Force Sudden Death");
	...
	AddFileToDownloadsTable("sound/TopNotchClan/shame.mp3");
	new Handle:topmenu;
...
/*
public Action:Command_SuddenDeath(client, args)
{
	FireEvent(CreateEvent("teamplay_suddendeath_begin"));
}
*/
...
public Action:Command_SwapTeam(client, args)
{
	...
	/**
	Block !swapteam from donators during sudden death
	*/
	if (suddendeath)
	{
		ReplyToCommand(client, "\x01\x04[SM]\x01 Cannot Swap Teams during Sudden Death!");
		return Plugin_Handled;
	}
	/**
	Block !swapteam from donators after scramble
	*/
	if (GS_IsClientTeamChangeBlocked(client, 1))
	{
		ReplyToCommand(client, "\x01\x04[SM]\x01 You cannot swap teams after a scramble! FOR SHAME!");
		if (RateShame) {
			EmitSoundToAll(Shame, _, _, _, _, 1.0);
			RateShame = false;
			CreateTimer(300.0, Timer_PlaySound, TIMER_FLAG_NO_MAPCHANGE);
		}
		return Plugin_Handled;
	}
	if (RoundTeamSwaps[client]++ >= 1)
	{
		ReplyToCommand(client, "\x01\x04[STM]\x01 Command blocked. You have already swapped teams once this round!");
		return Plugin_Handled;
	}
	new iTimeLeft = TF2_GetRoundTimeLeft();
	if (iTimeLeft && iTimeLeft < 120)
	{
		ReplyToCommand(client, "\x01\x04[STM]\x01 Command blocked. No team changes with less than 2 minutes left in a round!");
		return Plugin_Handled;
	}
