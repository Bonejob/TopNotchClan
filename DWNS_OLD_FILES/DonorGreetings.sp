...
public OnPluginStart()
{
	...
	v_DefaultMessage = CreateConVar("sm_greetings_default", "{D}has joined the game.", "Default welcome message. {N} expands to client name");
...
public SettingsMenuHandler(client, CookieMenuAction:action, any:selection, String:buffer[], maxlen)
{
    if (action == CookieMenuAction_SelectOption)
    {
		...
		if (selection == 1)
		{
			...
			{
				AddMenuItem(menu, "", "{G} will make any text after it GREEN", ITEMDRAW_DISABLED);
				AddMenuItem(menu, "", "{LG} will make any text after it LIGHT GREEN", ITEMDRAW_DISABLED);
				AddMenuItem(menu, "", "{O} will make any text after it OLIVE", ITEMDRAW_DISABLED);
				AddMenuItem(menu, "", "{D} will make any text after it the DEFAULT color.", ITEMDRAW_DISABLED);
			}
		...
		else if (selection == 2)
		{
			if (g_iAdminDisabled[client] == 1)
				PrintToChat(client, "Your access to this command as been revoked.  You may only enable or disable the default greeting.")
			else
			{
				PrintToChat(client, "Your greeting message has been reset to the default.");
				ClientResetGreeting(client);
			}
		}
		else if (selection == 3)
		{
			PrintToChat(client, "Your greeting message has been disabled.");
			ClientDisableGreeting(client);
		}
		else if (selection == 4)
		{
			PrintToChat(client, "Your greeting message has been re-enabled.");
...
public OnAdminMenuReady(Handle:topmenu)
{
	...
	//new TopMenuObject:TopMen = CreateTopMenu(g_hAdminMenu);
...
public HandleMenuDisable(Handle:menu, MenuAction:action, client, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		if (iTarget == 0)
			PrintToChat(client, "[SM] Selcted player is no longer available.");
		else if (!CanUserTarget(client, iTarget))
			PrintToChat(client, "[SM] Unable to target this player.");
		else
...
Connect()
{
	if (SQL_CheckConfig("default"))
		SQL_TConnect(Connected, "default");
	else
		SetFailState("Can't find 'default' entry in sourcemod/configs/databases.cfg!");
}
...
SQL_CreateTables(Handle:hndl)
{
	...
	len += Format(query[len], sizeof(query)-len, "  `id` int(5) NOT NULL AUTO_INCREMENT,");
	len += Format(query[len], sizeof(query)-len, "  `SteamID` varchar(25) COLLATE utf8_unicode_ci NOT NULL,");
	len += Format(query[len], sizeof(query)-len, "  `Greeting` varchar(128) COLLATE utf8_unicode_ci NOT NULL,");
	len += Format(query[len], sizeof(query)-len, "  `ClientDisabled` int(1) NOT NULL DEFAULT '0',");
	len += Format(query[len], sizeof(query)-len, "  `AdminDisabled` int(1) NOT NULL DEFAULT '0',");
...
public OnClientPostAdminCheck(client)
{
	...
	if (!CheckCommandAccess(client, "greeting_override", ADMFLAG_RESERVATION))
	{
		...
		PrintToChatAll("%s has joined the game.", sName);
		return;
	}
...
PrintGreeting(client, String:message[], size, bool:bIsCustom)
{
	...
	decl String:FixThisShit[168];
	Format(FixThisShit, sizeof(FixThisShit), "\x01Donator \x03%s\x01 %s", name, message);
	if (!bIsCustom || GetConVarBool(v_AllowCustomColors))
	{
		ReplaceString(FixThisShit, size, "{D}", "\x01", false);	// Default color
		ReplaceString(FixThisShit, size, "{G}", "\x04", false);	//Bright  green
		ReplaceString(FixThisShit, size, "{LG}", "\x03", false);	//Light  green
		ReplaceString(FixThisShit, size, "{O}", "\x05", false);	//Olive
	}
	else	//Strip color codes out
	{
		ReplaceString(FixThisShit, size, "{D}", "", false);
		ReplaceString(FixThisShit, size, "{G}", "", false);
		ReplaceString(FixThisShit, size, "{LG}", "", false);
		ReplaceString(FixThisShit, size, "{O}", "", false);
	}
	//ReplaceString(FixThisShit, size, "{N}", name, false);
	ReplaceStringEx(FixThisShit, size, "{N}", name, -1, -1, false);	//will only replace one {N}
	PrintToChatAll("%s", FixThisShit);
}
...
ClientDisableGreeting(client)
{
	...
	GetClientAuthString(client, SteamID, sizeof(SteamID));
	decl String:query[512];
	Format(query, sizeof(query), "INSERT INTO `DonorGreetings` (`SteamID` ,`ClientDisabled`) VALUES ('%s', '1') ON DUPLICATE KEY UPDATE `ClientDisabled` = '1'", SteamID);
	if (bDebug)
			LogError(query);
	SQL_TQuery(g_Database, ProcessGreeting, query);
...
ClientSetGreeting(client, String:message[], size)
{
	...
	Format(query, sizeof(query), "INSERT INTO `DonorGreetings` (`SteamID` ,`Greeting`) VALUES ('%s', '%s') ON DUPLICATE KEY UPDATE `Greeting` = '%s'", SteamID, buffer, buffer);
	if (bDebug)
			LogError(query);
	SQL_TQuery(g_Database, ProcessGreeting, query);
	if (GetConVarBool(v_AllowCustomColors))
	{
		ReplaceString(message, size, "{D}", "\x01", false);	// Default color
		ReplaceString(message, size, "{G}", "\x04", false);	//Bright  green
		ReplaceString(message, size, "{LG}", "\x03", false);	//Light  green
		ReplaceString(message, size, "{O}", "\x05", false);	//Olive
	}
	else
	{
		ReplaceString(message, size, "{D}", "", false);
		ReplaceString(message, size, "{G}", "", false);
		ReplaceString(message, size, "{LG}", "", false);
		ReplaceString(message, size, "{O}", "", false);
	}
	decl String:name[32];
	GetClientName(client, name, sizeof(name));
