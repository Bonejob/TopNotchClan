...
public OnPluginStart() {
	...
	RegAdminCmd("sm_rick", Command_ShowMOTD, ADMFLAG_KICK, "sm_motd <#userid|name>");
	RulesURL = CreateConVar("sm_rick_url","http://www.topnotchclan.com/server/rick.html","Set this to the URL of your MOTD/Rules.");
...
public Action:Command_Say(client, args) {
	...
	if (strcmp(message, "!rick", false) == 0 || strcmp(message, "!rickroll", false) == 0) {
...
public PerformMOTD(client, target) {
	if (client != target) {
		...
		PrintToChatAll("%c[RICK] %c%s %s %s %s",COLOR_GREEN,COLOR_DEFAULT,clientName,"just rick rolled",targetName,"!");
	}
