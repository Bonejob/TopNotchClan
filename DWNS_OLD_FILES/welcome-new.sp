...
#include <sourcemod>
#include <sdktools>
#define PLUGIN_VERSION "5-2-2012"
...
public Action:Timer_ChatMessageDelayed(Handle:timer, any:userID)
{
	...
	if (DatabaseIntact() && client > 0 && IsClientInGame(client))
	{
		...
		Format(query, sizeof(query), "SELECT `vb_user`.`userid`, `vb_subscriptionlog`.`expirydate`, `vb_subscriptionlog`.`subscriptionid` FROM `vb_user` LEFT JOIN `vb_subscriptionlog` ON `vb_user`.`userid`=`vb_subscriptionlog`.`userid` LEFT JOIN `vb_userfield` ON `vb_user`.`userid`=`vb_userfield`.`userid` WHERE `vb_userfield`.`field6` = '%s' AND `vb_subscriptionlog`.`status` = 1 AND `vb_subscriptionlog`.`expirydate` > %i ORDER BY `vb_subscriptionlog`.`expirydate` DESC LIMIT 1", SteamID_ESC, GetTime());
...
public Action:Command_Admins(client, args)
{
	...
	if (id != INVALID_ADMIN_ID)
	{
		...
		AddMenuItem(hMenu, "2", "VIP Settings");
